...
const uploadExcelOnFirebase = (pdfUri, pdfName = '') => {
    return new Promise(resolve => {
        var date = new Date().getTime();
        uriToBlob(pdfUri)
            .then(res => {
                const Storage = fire
                    ...
                    .child(
                        pdfName.indexOf('.') > 0
                            ? pdfName.substring(0, pdfName.indexOf('.')) + '_pdf' + date.toString() + '.xlsx'
                            : pdfName + '_pdf' + date.toString() + '.xlsx'
                    );
                Storage.put(res, {
                    contentType: '*/*',
                })
...
export {uriToBlob, uploadImageOnFirebase, isDefined,uploadPdfOnFirebase,uploadExcelOnFirebase};
